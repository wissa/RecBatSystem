...
const uint8_t SLAVE_ADDRESS = 0x10; // Default I2C Address
...
volatile uint8_t newAddress = SLAVE_ADDRESS;
volatile uint8_t currentI2CAddress = SLAVE_ADDRESS;
...
const int ADD_PINS[] = {25, 26, 14, 15};
...
const unsigned long LED1Interval = 1000; // 300 ms
const unsigned long PrintInterval = 2000; // 300 ms
...
void onI2CReceive(int numBytes) {
  ...
  if (lastCommand == 0xA0 && numBytes >= 2) {
    ...
    if (newAddress >= 0x10 && newAddress <= 0x7F) {
      ...
      Serial.println(newAddress, HEX);
        ...
        // Handle I²C address change if requested
  ...
  } else if (lastCommand == 'C' && numBytes >= 13) {
    ...
    saveConfigurations();
    lastCommand = 0;
  } else if (lastCommand == 'Q' && numBytes >= 25) {
    ...
    saveConfigurations();
    lastCommand = 0;
  } else if (lastCommand == 'S' && numBytes >= 2) {
    ...
    //PORTB = MySW_Config.SW_Config; // Update PORTB immediately
    lastCommand = 0;
...
void loop() {
  ...
  if (currentMillis - previousLED2Toggle >= LED2Interval && currentI2CAddress != SLAVE_ADDRESS) {
...
void initializePins() {
  ...
  for(auto pin : ADD_PINS) pinMode(pin, INPUT);
}
...
void loadConfigurations() {
  ...
  for (int i = 0; i < MAX_PATTERNS  ; i++) {
    ...
    if (isnan(MySW_Config.SW_Config[i])) {
      MySW_Config.SW_Config[i] = 0xFF;
      EEPROM.put(SW_ADDR+i, MySW_Config.SW_Config[i]);
    }
  }
    
  PORTB = 0x00; //MySW_Config.SW_Config[0];
}
...
void processChannel(int channel) {
  ...
  switch(channel) {
    case 0: { // Current measurement with ±2.5V reference
      ...
      BatCurrent = (calibratedCurrent < 0.001f) ? 0.0f : calibratedCurrent;
      break;
    }
...
void enterBatterySettingMode() {
  ...
  Serial.println(F("Current Battery Setting ..."));
